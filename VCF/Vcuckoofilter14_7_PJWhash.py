...
import exceptions
import hashutils_PJWHash


class VCuckooFilter14_7_PJWhash(object):
    """
    Cuckoo Filter class.
    ...
    def _get_index(self, item):
        index = hashutils_PJWHash.hash_code(item) % self.capacity
        return index

    def _get_alternate1_index(self, index, fingerprint):
        alt_index1 = (index ^ (hashutils_PJWHash.hash_code(fingerprint) & 0b01010101010101)) % self.capacity
        return alt_index1

    def _get_alternate2_index(self, index, fingerprint):
        alt_index2 = (index ^ (hashutils_PJWHash.hash_code(fingerprint) & 0b10101010101010)) % self.capacity
        return alt_index2

    def _get_alternate3_index(self, index, fingerprint):
        alt_index3 = (index ^ hashutils_PJWHash.hash_code(fingerprint)) % self.capacity
        return alt_index3
    ...
    def I_insert0(self, item):
        ...
        filter is full.
        """
        fingerprint = hashutils_PJWHash.fingerprint(item, self.fingerprint_size)
    ...
    def delete1(self, item):
        ...
        """
        fingerprint = hashutils_PJWHash.fingerprint(item, size=self.fingerprint_size)
        i = self._get_index(item)
        j = self._get_alternate3_index(i, fingerprint)
