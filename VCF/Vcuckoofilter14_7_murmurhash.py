...
class VCuckooFilter14_7_murmurhash(object):
    """
    Cuckoo Filter class.
    ...
    def _get_alternate1_index(self, index, fingerprint):
        alt_index1 = (index ^ (hashutils_murmurhash.hash_code(fingerprint) & 0b01010101010101)) % self.capacity
        return alt_index1

    def _get_alternate2_index(self, index, fingerprint):
        alt_index2 = (index ^ (hashutils_murmurhash.hash_code(fingerprint) & 0b10101010101010)) % self.capacity
        return alt_index2

    def _get_alternate3_index(self, index, fingerprint):
        alt_index3 = (index ^ hashutils_murmurhash.hash_code(fingerprint)) % self.capacity
        return alt_index3
