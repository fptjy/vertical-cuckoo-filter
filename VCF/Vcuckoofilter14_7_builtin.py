...
import exceptions
import hashutils_builtin


class VCuckooFilter14_7_builtin(object):
    """
    Cuckoo Filter class.
    ...
    def _get_index(self, item):
        index = hashutils_builtin.hash_code(item) % self.capacity
        return index

    def _get_alternate1_index(self, index, fingerprint):
        alt_index1 = (index ^ (hashutils_builtin.hash_code(fingerprint) & 0b01010101010101)) % self.capacity
        return alt_index1

    def _get_alternate2_index(self, index, fingerprint):
        alt_index2 = (index ^ (hashutils_builtin.hash_code(fingerprint) & 0b10101010101010)) % self.capacity
        return alt_index2

    def _get_alternate3_index(self, index, fingerprint):
        alt_index3 = (index ^ hashutils_builtin.hash_code(fingerprint)) % self.capacity
        return alt_index3
    ...
    def I_insert0(self, item):
        ...
        filter is full.
        """
        fingerprint = hashutils_builtin.fingerprint(item, self.fingerprint_size)
    ...
    def delete1(self, item):
        ...
        """
        fingerprint = hashutils_builtin.fingerprint(item, size=self.fingerprint_size)
        i = self._get_index(item)
        j = self._get_alternate3_index(i, fingerprint)
